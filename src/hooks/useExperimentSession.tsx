...
export const useExperimentSession = () => {
  ...
  const saveSessionToFile = async (session: ExperimentSession) => {
    try {
      ...
      const jsonlContent = [
        ...
        ...session.events.map(event => JSON.stringify({ type: 'event', session_id: session.session_id, ...event }))
      ].join('\n');
  ...
  const getSessionInfo = useCallback(() => {
    ...
    return {
      ...currentSession,
      duration_seconds: (new Date().getTime() - new Date(currentSession.start_time).getTime()) / 1000,
